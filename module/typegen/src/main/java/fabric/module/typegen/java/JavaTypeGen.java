...
public class JavaTypeGen implements TypeGen {
    ...
    public JavaTypeGen() {
        try {
            mapper = MapperFactory.getInstance().createMapper("JavaMapper");
    ...
    @Override public void generateSourceFiles(Workspace workspace) throws Exception {
        /*
    ...
    @Override public void addSimpleType(FSimpleType type, FElement parent) throws Exception {
        /*
        ...
        if (generatedElements.containsKey(type.getName())) {
            ...
            Check restrictions
             */
            checkRestrictions(type);    // TODO: Einschränkungen müssen in den Container übernommen werden!
            ...
            if (FSchemaTypeHelper.isArray(parent)) {  // Element is an array
                current.addElementArray(mapper.lookup(type), type.getName(), parent.getMaxOccurs());
            ...
            } else {    // Element is a variable of a simple datatype
                current.addElement(mapper.lookup(type), type.getName());
            }
    ...
    @Override public void generateNewContainer(FComplexType type) {
        ...
        for (FSchemaAttribute attr : attributes) {
            newBuilder.addAttribute(mapper.lookup(attr.getSchemaType()), attr.getName());
        }
